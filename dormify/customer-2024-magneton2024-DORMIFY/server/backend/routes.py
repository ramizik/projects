...
def build_filter_query(semester_rate, meal_plan, dorm_type, amenities, group_exclusive):
    """
    Constructs the SQL query based on provided filters.

    Args:
        semester_rate (int): Maximum semester rate.
        meal_plan (int): Maximum meal plan.
        dorm_type (list): List of dorm types.
        amenities (list): List of amenities.
        group_exclusive (list): List of group exclusivity options.

    Returns:
        tuple: (query string, list of parameters)
    """
...
def filter_dormitories():
    ...
    try:
        ...
        dorm_type = (request.args.getlist('dormType')
                     if request.args.getlist('dormType') != [''] else [])
        amenities = (request.args.getlist('amenities')
                     if request.args.getlist('amenities') != [''] else [])
        group_exclusive = (request.args.getlist('groupExclusive')
                           if request.args.getlist('groupExclusive') != [''] else [])
        ...
        # Build the query and parameters using the helper function
        res = build_filter_query(semester_rate, meal_plan, dorm_type, amenities, group_exclusive)
        query, params = res
